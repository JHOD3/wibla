  ...
  class mdbRange {
    ...
    rangeChange() {
      ...
      this.$range.on('change', e => {
        ...
        this.options.single.value !== undefined ? $thumbValue.html(`${$etarget.val()} ${this.options.single.value.symbol}`) : $thumbValue.html('$'+ new Intl.NumberFormat("de-DE").format($etarget.val()) );
      });
    }
    ...
    inputMouseDown() {
      ...
      this.$range.on('input mousedown touchstart', e => {
        ...
        this.options.single.value !== undefined ? $thumb.find('.value').html(`'$'+${$etarget.val()} ${this.options.single.value.symbol}`) : $thumb.find('.value').html('$'+ new Intl.NumberFormat("de-DE").format($etarget.val()));
      });
    }
